...
async fn test() -> Result<()> {
    ...
    let values1: serde_json::Value = values_ui.get_values::<TestDb>(&inputs1).await?.into();
    let expected1 = json!({
        "config": {
            "required_enum": "value2",
        }
    ...
    assert!(std::matches!(
        values_ui.get_values::<TestDb>(&inputs2).await,
    ...
    let values3: serde_json::Value = values_ui.get_values::<TestDb>(&inputs3).await?.into();
    let expected3 = json!({
        "config": {
            "required_enum": "value3",
            "required_dependent_num": 5,
